  ...
  >
    ...
    <div className="flex items-center gap-2.5 flex-wrap">
      ...
      {title === 'Device Portfolio' && (
        <span 
          id="badge-new-device-portfolio"
          className="px-1.5 py-0.5 rounded text-[8px] font-black uppercase tracking-wider bg-gradient-to-r from-red-500 via-yellow-500 via-green-500 to-blue-500 text-white shadow-xs"
        >
          NEW
        </span>
      )}
                  ...
                  {hasSubGroups && (
                    <div className="space-y-2">
                      {subGroupEntries.filter(([_, sub]) => !isSearching || (sub.items.length > 0 || (sub.nested && Object.values(sub.nested).some(l => l.length > 0)))).map(([subCat, subData]) => {
                        ...
                        const isGooglebook = subCat === 'Googlebook';
                        return (
                          <div 
                            key={subCat} 
                            className={isGooglebook 
                              ? "p-[2px] rounded-xl bg-gradient-to-r from-red-500 via-amber-400 via-emerald-400 via-blue-500 to-purple-500 shadow-lg shadow-purple-500/15 overflow-hidden transition-all" 
                              : "border border-gray-800/50 rounded-lg overflow-hidden"
                            }
                            id={isGooglebook ? "googlebook-category-dropdown" : undefined}
                          >
                            <div 
                              onClick={() => setSubSectionsOpen(prev => ({...prev, [subCat]: !prev[subCat]}))}
                              className={isGooglebook
                                ? "flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-red-950/60 via-amber-950/50 via-emerald-950/50 via-blue-950/60 to-purple-950/60 hover:from-red-900/70 hover:to-purple-900/70 cursor-pointer transition-all group rounded-[10px]"
                                : "flex items-center gap-2 px-4 py-2 bg-[#18191A] hover:bg-[#1E1F20] cursor-pointer transition-colors group"
                              }
                            >
                              {isSubOpen ? (
                                <ChevronDown className={`w-3.5 h-3.5 ${isGooglebook ? 'text-amber-300' : 'text-gray-600'}`} />
                              ) : (
                                <ChevronRight className={`w-3.5 h-3.5 ${isGooglebook ? 'text-amber-300' : 'text-gray-600'}`} />
                              )}
                              
                              {isGooglebook ? (
                                <div className="flex items-center gap-2 flex-wrap">
                                  <span className="text-[11px] font-bold uppercase tracking-widest text-white drop-shadow-xs">
                                    {subCat}
                                  </span>
                                  <span className="px-1.5 py-0.5 rounded text-[8px] font-black uppercase tracking-wider bg-gradient-to-r from-red-500 via-yellow-500 via-green-500 to-blue-500 text-white shadow-xs">
                                    NEW
                                  </span>
                                </div>
                              ) : (
                              ...
                              <span className={isGooglebook 
                                ? "text-[9px] text-amber-200 font-mono font-bold ml-auto bg-black/50 px-2 py-0.5 rounded-full border border-amber-400/30" 
                                : "text-[9px] text-gray-700 ml-auto font-mono"
                              }>
                                ({subData.items.length + (subData.nested ? Object.values(subData.nested).reduce((acc, l) => acc + l.length, 0) : 0)})
                              </span>
                            </div>
                            
                            {isSubOpen && (
                              <div className={isGooglebook ? "bg-[#131314] p-3 space-y-3 rounded-b-[10px]" : "bg-[#131314] p-3 space-y-3"}>
